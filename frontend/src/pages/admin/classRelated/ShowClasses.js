...
const ShowClasses = () => {
  ...
  const SclassButtonHaver = ({ row }) => {
    const actions = [
      { icon: <PostAddIcon />, name: 'Add Segments', action: () => navigate("/Admin/addsubject/" + row.id) },
    ...
    return (
      <>
        <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'center' }}>
          <Tooltip title="Add Employees & Segments">
            <IconButton
              onClick={handleClick}
              size="small"
              sx={{ ml: 2 }}
  ...
  const actions = [
    {
      icon: <AddCardIcon color="primary" />, name: 'Add New Project',
